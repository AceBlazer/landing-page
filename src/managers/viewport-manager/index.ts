...
class ViewportManager {
  ...
  private _attachEvents() {
    ...
    const scrollEventOptions = supportsPassiveEvents
      ? { passive: true }
      : false;
    if (this._state.scrollElement) {
      // need to listen for scroll event on window object for document.scrollingElement elements
      const listenerTarget =
        documentScrollingElement === this._state.scrollElement
          ? window
